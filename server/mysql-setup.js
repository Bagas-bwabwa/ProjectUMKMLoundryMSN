...
async function setupMySQLDatabase() {
  try {
    ...
    // Read SQL setup file
    const sqlFilePath = path.join(__dirname, 'mysql-setup.sql');
    ...
    console.log('📊 Tables created: users, customers, services, items, transactions, transaction_services, expenses');
    console.log('👤 Sample users:');
    console.log('   - Admin: admin@laundry.com / password');
    console.log('   - Cashier: kasir@laundry.com / password');
    console.log('📦 Sample services and items inserted');
    ...
    if (error.code === 'ER_ACCESS_DENIED_ERROR') {
      ...
      console.log('\n💡 Tips: Check if the database user has privileges to create databases');
    }
